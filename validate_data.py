...
SUMMARY_PATH = Path(__file__).parent / "public" / "data-quality" / "validation-summary.json"
...
def generate_report(all_issues, states_processed):
    ...
    # Machine-readable provenance for the deployed UI. This is deliberately
    # generated by the same validation pass that gates deployment, preventing
    # the website from claiming a stale or separate quality status.
    state_summary = {}
    for state in sorted(states_processed):
        state_issues = [i for i in all_issues if i.state == state]
        state_summary[state] = {
            "critical": sum(i.severity == Issue.CRITICAL for i in state_issues),
            "warning": sum(i.severity == Issue.WARNING for i in state_issues),
            "info": sum(i.severity == Issue.INFO for i in state_issues),
        }
    summary_payload = {
        "schema_version": 1,
        "generated_at": generated_at.isoformat(timespec="seconds"),
        "status": "issues_detected" if all_issues else "clean",
        ...
        "report_url": "https://github.com/ksabhinav/projectfiner/blob/main/DATA_VALIDATION_REPORT.md",
    }
