#!/usr/bin/env python3
"""
Extract district-wise tables from the Uttarakhand Wayback PDF haul.

Mirror of db/extract_wayback_kerala.py for Uttarakhand's 13 districts.
Run AFTER db/fetch_wayback_pdfs.py uttarakhand slbcuttarakhand.com.

Uttarakhand Source URL: https://slbcuttarakhand.com (convenor: SBI)
Notes:
- Source values likely in Crores; downstream normalization should ×100
  to Lakhs (verify per-PDF).
"""
...
# Uttarakhand's 13 districts. Includes alias for Hardwar/Haridwar,
# US Nagar variations.
UK_DISTRICT_CANON = {
    'almora': 'Almora', 'bageshwar': 'Bageshwar',
    'chamoli': 'Chamoli', 'champawat': 'Champawat',
    'dehradun': 'Dehradun', 'dehradan': 'Dehradun',
    'haridwar': 'Haridwar', 'hardwar': 'Haridwar',
    'nainital': 'Nainital',
    'paurigarhwal': 'Pauri Garhwal', 'pauri': 'Pauri Garhwal',
    'pithoragarh': 'Pithoragarh', 'rudraprayag': 'Rudraprayag',
    'rudra prayag': 'Rudraprayag',
    'tehrigarhwal': 'Tehri Garhwal', 'tehri': 'Tehri Garhwal',
    'udhamsinghnagar': 'Udham Singh Nagar',
    'udhamSinghnagar': 'Udham Singh Nagar',
    'usnagar': 'Udham Singh Nagar',
    'udamsinghnagar': 'Udham Singh Nagar',
    'uttarkashi': 'Uttarkashi', 'uttarkasi': 'Uttarkashi',
}
...
def canon_district(s: str | None) -> str | None:
    ...
    if not key or len(key) < 4:
        return None
    if key in UK_DISTRICT_CANON:
        return UK_DISTRICT_CANON[key]
    for alias, canon in UK_DISTRICT_CANON.items():
        ...
        for pi, page in enumerate(pdf.pages[:8]):
            for t in page.extract_tables() or []:
                ...
                for col in range(min(4, ncols)):
                    ...
                    # Uttarakhand has 13 districts; ≥7 for a confident match.
                    if len(unique) >= 7:
