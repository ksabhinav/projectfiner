...
│
├── scripts/
│   ├── build_og_image.py                  # Site-wide OG card regen (Playwright/Chrome, template: og_card_template.html)
...
**CI gate (exact-identity waivers)**: `.github/workflows/deploy.yml` runs `validate_data.py --waivers .github/validation-waivers` as a `validate` job that `build` depends on. Every accepted legacy critical has a SHA-256 fingerprint over state, rule, district, field, and period. Any new identity fails independently, even if another issue disappears from the same bucket. Waivers expire and resolved fingerprints should be removed in review; never regenerate the ledger wholesale. NEVER run `run_validation.sh` expecting `set -e` semantics — it deliberately doesn't `set -e`, because exit-1-on-critical is the validator's designed signal, not a script failure.
...
60. **OG image regeneration**: `python3 scripts/build_og_image.py` fills `scripts/og_card_template.html` with a district choropleth built from the latest `public/indicators/digital_transactions/` quarter (the map's own ramp + quantile breaks; unjoined districts stay no-data grey) and screenshots it with headless Chrome via Playwright → `public/og-image.png`. Chrome is used because cairosvg silently dropped the Google Fonts (Fraunces/Plex Mono) and fell back to Helvetica. Re-run after a new PhonePe quarter so the caption/map stay current, and bump the `?v=` on the og:image URL in `BaseLayout.astro` so WhatsApp re-fetches. Keep the PNG 1200×630 and well under 300 KB (WhatsApp drops larger preview images).
