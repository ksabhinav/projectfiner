...
**Chhattisgarh Source**: [slbcchhattisgarh.com](https://slbcchhattisgarh.com) (migrated May 2026 — dedicated site with per-meeting Excel data-tables, replaces old slbcne.nic.in fallback). 15 quarters (Sep 2022 → **Mar 2026**). v2 extractor: `slbc-data/chhattisgarh/extract_chhattisgarh_v2.py`. Meeting→quarter map lives in `MEETING_QUARTERS_EXT` (101 = Dec 2025, 102 = Mar 2026); a new meeting needs an entry there plus its `data-table` zip unpacked to `data-tables/<n>/`. **Always run `--all`** — see gotcha #88.
...
**Madhya Pradesh Source**: [slbcmadhyapradesh.in](https://www.slbcmadhyapradesh.in/slbc-meeting.aspx) — Excel data-tables per meeting. 16 quarters (Dec 2016 → **Mar 2026**) via `slbc-data/madhya-pradesh/scrape_mp_archive.py` + Wayback backfill. Per-quarter live ingest: **`db/add_mp_quarter_xlsx.py <xlsx> <YYYY-MM>`** (reads the `CD Ratio_3(ii)Dist` sheet; shares its district resolver, CD sanity check and output writers with `db/normalize_wayback_madhya_pradesh.py` via that module's `write_outputs()`, so the live and Wayback paths cannot drift). **MP SLBC publishes district-level data ONLY for CD ratio** — all other metrics are bank-wise. Branch counts for MP come from the RBI Banking Outlets snapshot indicator instead.
...
**Himachal Pradesh Source**: [slbchp.com](https://slbchp.com) (PNB convenor, Shimla — uses its own dedicated domain, NOT the slbchp.pnb.bank.in pattern). The 179th SLBC Agenda (Dec 2025) ships a 4-XLSX bundle with ~30 district-wise annexures covering all 12 districts (Bilaspur, Chamba, Hamirpur, Kangra, Kinnaur, Kullu, Lahul & Spiti, Mandi, Shimla, Sirmaur, Solan, Una). **15 categories** captured per quarter; **Dec 2025 + Mar 2026** ship the XLSX bundle (11 further quarters came from `db/normalize_wayback_hp.py`). Ingest a new quarter with `python3 extract_hp.py --raw-dir raw/<dir> --quarter YYYY-MM` — it upserts into the existing outputs rather than replacing them (it used to be single-quarter and would wipe the Wayback history). Older quarters are PDF-only and would need a separate pdfplumber pipeline to backfill (like Uttarakhand). **Annexure layouts drift between quarters — see gotcha #89.** **Source already in Rs. Lakhs** — NO Crore→Lakh ×100 conversion applied (unlike Haryana / UP / J&K which need ×100).
...
| Madhya Pradesh | Mar 2026 | 16 | 55 |
| Uttar Pradesh | Dec 2025 | 25 | 75 |
| Goa | Dec 2025 | 23 | 2 |
| Delhi (NCT) | Dec 2025 | 13 | 11 |
| Himachal Pradesh | Mar 2026 | 13 | 12 |
...
- **Progressive loading (indicator data)**: indicator files (~50-100KB each) are fetched on demand per indicator/quarter, not all 31 state timeseries up front. BUT first load is NOT ~150 KB — the map also fetches `data/district_boundaries.geojson` (~5 MB after the July 2026 simplification, down from 18 MB; `scripts/simplify_district_boundaries.py`) and `Maps/india-outline.geojson` (~1.2 MB) unconditionally before it paints. So real first-load ≈ 6 MB + manifest (8 KB) + `district_lgd_codes.json` (92 KB). Shrinking the district geojson further is the biggest remaining first-load win. See [Progressive Loading](#progressive-loading-indicators) section.
...
The homepage map uses **pre-aggregated indicator files** instead of loading all 31 state timeseries JSONs — the per-indicator data payload drops from ~19 MB to ~50-100 KB per selection. (This does NOT make total first load ~150 KB: the ~5 MB `district_boundaries.geojson` + ~1.2 MB india outline still load up front — see the corrected Homepage Map note above.)
...
`validate_data.py` — automated quality checks across all 31 states.
...
**CRITICAL — the validator was blind to 29 of 31 states until July 2026.** Every check gated on `isinstance(val, (int, float))`, but only Goa serialises values as JSON numbers — the other 30 states store them as strings ("2613.61"), so the checks silently skipped them. And `is_ratio_field` matched the whole field name, so `credit_deposit_ratio__total_deposit`/`total_advance` (the most important monetary fields) were treated as ratios and never range-checked. Both are fixed (`numeric_value()` coercion + metric-half-only ratio test). Effect: critical findings went from 14 to ~29,000. Most are legitimate fiscal-year resets, so absolute counts are unusable as a gate.

**CI gate (regression-based)**: `.github/workflows/deploy.yml` runs `validate_data.py --baseline .github/validation-baseline.json` as a `validate` job that `build` depends on. The baseline holds per-(state, issue_type) COUNTS; the gate fails only when a bucket's count RISES, so a bad extraction is caught while pre-existing debt doesn't block deploys. After fixing something, lower the number (or regenerate with `--write-baseline`). NEVER run `run_validation.sh` expecting `set -e` semantics — it deliberately doesn't `set -e`, because exit-1-on-critical is the validator's designed signal, not a script failure.
...
34. **NE portal scraper FIXED (June 2026)**: `onlineslbcne.nic.in` was revamped ~May 2026: (a) state selection is no longer cookie-based — the server sets NO cookies; GETting the state landing page `/{CODE}` (e.g. `/ME`) binds the state **server-side keyed by client IP**, and the form page's "Go Back" link (`<a href="ME">`) reveals the active binding; (b) the old form/action pairs (`districtwiseCDr.php` → `distwiseDepAdvReport.php`, lowercase `districtwisekcc.php` etc.) are decommissioned — they still render and accept POSTs but always return an all-zero Grand-Total table; (c) POSTs with any unexpected field (e.g. adding `state=ME`) bounce to `error.php`; POSTs with no state bound to your IP return `alert('Invalid Data')`. `scrape_onlineslbc.py` was fixed accordingly: `select_state()` GETs `/{CODE}`, `extract_active_state()` verifies the Go-Back link before every POST (re-selects if flipped), and `REPORTS` was remapped to the 36 live district-wise form pages. Verified: ME `districtwiseCdrdata.php` + `districtwisePMJDY.php` for **Mar 2026** (published!) return 13 districts (incl. new EASTERNWESTKHASIHILLS) with real data. CAVEAT: per-IP server-side state binding means never scrape two states concurrently from the same IP. **Sibling scrapers (June 2026)** — five total were reconciled to the revamp: (1) `scrape_missing_quarters.py` (root) and (2) `public/slbc-data/scrape_portal_mn_mz_me.py` were **ported** to the new mechanism (GET `/{CODE}` + `extract_active_state`/Go-Back verification before every POST, remapped REPORTS, `Invalid Data` guard); (3) `slbc-data/scrape_ne_blockwise.py` already used live `blockwise*.php` endpoints with native form submit, **hardened** with Playwright Go-Back binding verification (it's an auxiliary blockwise scraper, finer-grained than the district-level FINER pipeline); (4) `slbc-data/scrape_ne_all_reports.py` (the Playwright producer of `ne_full_scrape.json`, using dead `districtwiseCDr.php` + `form.action` overrides to old `*report.php` pages) and (5) `slbc-data/rescrape_mn.py` (one-off MN contamination patch) are **DEPRECATED** with header banners pointing at `scrape_onlineslbc.py` — don't run them. (`scripts/check_new_quarters.py` and `db/build_wayback_manifest.py` only parse listing-page URLs, not the form-POST mechanism — unaffected.)
...
44. **Progressive loading fallback = merge-and-tag (NOT replace)**: When the selected quarter has fewer than 300 districts, `loadIndicatorData()` keeps that quarter's districts with their real values and fills ONLY the missing districts from nearest later quarters (up to a 12-quarter window), tagging each filled record with a per-record `_fromQuarter`. `buildDistrictLookup` reads that tag so fresh states show their real quarter and filled states disclose "<quarter> (latest available)" in the tooltip. It used to discard the whole file and swap in an earlier quarter, which hid states that DID report the selected quarter (Karnataka/Ladakh vanished when Mar 2026 fell back to Dec 2025, which lacks them) and mislabelled everything shown. Verified: Mar 2026 + CD Ratio yields 530 merged districts, KA + Ladakh untagged with real values. Follow-up: dispatch the effective quarter on `finer:data-loaded` so MapLegend can add an aggregate "showing <older> for some states" note.
...
87. **Wayback Machine truncates PDFs >1MB at CAPTURE time**: When the Internet Archive's crawler captures a PDF/binary, it caps the response at exactly 1,048,576 bytes (1 MB) — the response header reads `warning: 299 wayback content truncated by "length"` and `x-archive-orig-x-crawler-content-length: <real-size>` reveals the true original. THIS IS NOT FIXABLE BY RE-DOWNLOADING THE SAME SNAPSHOT — the snapshot itself was truncated at the source. May 2026 Kerala haul: 100 of 408 PDFs hit this cap (mostly 2019-2023 SLBC books, COVID-19 advisories, PMFBY annexures, where originals were 5-20 MB). `db/fetch_wayback_pdfs.py` detects this via the warning header and marks `status: "truncated"` in the manifest so re-runs don't waste bandwidth, and per-state extractors should `if e.get('status') == 'truncated': continue` to skip them cleanly. Recovery paths: (a) probe CDX for ALTERNATE timestamps of the same URL with `&fl=timestamp,length` — sometimes an earlier capture had a higher cap or was untruncated; (b) fetch from the live SLBC site if it still serves the file; (c) accept the loss. The partial 1MB header bytes are sometimes parseable by pdfplumber for the first few pages (cover, TOC) but tables on later pages are gone.
88. **Chhattisgarh's extractor MUST be run with `--all`**: `extract_chhattisgarh_v2.py` builds `all_quarters` from scratch on every run and `build_complete_json()` writes it wholesale, so `python3 extract_chhattisgarh_v2.py 102` does not add meeting 102 — it produces a `chhattisgarh_complete.json` containing ONLY meeting 102 and silently drops the other 14 quarters. Always `--all` (every meeting in `data-tables/`), and diff the result against the committed files before promoting. July 2026 practice: extract to `--output-dir <tmp>` first, verify pre-existing quarters are unchanged, then copy over.
89. **HP CQR annexure bundles are neither file-stable nor column-stable between quarters**: two independent failure modes, both silent. (a) FILE drift — the same annexure moves between the four workbooks (`ANNEX-58`/`ANNEX-60` shipped in `CQR-ANNEXURES1To20` for Dec 2025 but in `51To74` for Mar 2026; `ANNEX-49A` moved `51To74`→`33TO50`), and filenames carry per-quarter suffixes (`" Dec 25"`, `" (2)"`). `extract_hp.py` now resolves sheets via `build_sheet_index()`, scanning every XLSX, instead of hardcoding filenames. (b) COLUMN drift — Mar 2026's `ANNEX-52` inserted a "Target" No./Amt pair ahead of every "Disbursements" block, shifting each field one block left; the Dec 2025 indices then read all-zero target columns as `nrlm_shg_disbursement` and reported TOTAL NRLM disbursements as `nrlm_women_shg_disbursement` (a 60-760x jump). Handled by `ANNEXURE_OVERRIDES[quarter][annex_key]`. **Before trusting any new HP quarter, diff the header rows above `data_start_row` against the previous quarter and check `validate_data.py --state himachal-pradesh` for 10x jumps** — the mis-mapping is invisible in the row counts (still a clean 12 districts) and only the validator catches it. Note Dec 2025's field NAMES are themselves suspect (`nrlm_women_shg`/`nrlm_individual` actually hold the NULM-SEP(I)/(G) blocks); kept as-is for series continuity, renaming is a separate correction.
90. **HP Mar 2026 ships ANNEX-06's C.D RATIO columns EMPTY**: deposits and advances are present and continuous, but columns 11-14 (previous/current CD ratio) are blank in the source workbook. `extract_hp.py` derives `overall_cd_ratio = advance/deposit*100` when the printed value is absent, bounded to 5-400% (same convention as `map_cd` in `db/normalize_wayback_madhya_pradesh.py`, and the same reasoning as gotcha #51). Without this HP would be invisible on the CD-ratio choropleth for the quarter. Sanity check that the quarter is aligned at all: HP's ANNEX-06 also prints a "previous quarter" column — all 12 districts' Mar 2026 previous-quarter deposits equal Dec 2025's current-quarter values exactly.
91. **Many live extractors DESTROY quarter history on re-run — they build output from scratch and write it wholesale.** All 7 Wayback normalizers upsert correctly, but the live per-state extractors overwrite. Confirmed destructive if re-run naively: `slbc-data/bihar/extract_bihar.py` and `slbc-data/bihar/extract_bihar_v2.py` (`extract_bihar.py <one.pdf>` collapsed 8 quarters → 1, killing the Wayback 2016-03), `public/slbc-data/extract_kerala.py` (dropped 9 historical 2010-2013 quarters that have no local PDF), `extract_chhattisgarh.py`/`_v2.py` (gotcha #88), `extract_jharkhand.py`, `extract_odisha.py`, `public/slbc-data/extract_karnataka.py`. **Fixed with load-then-upsert: extract_hp.py, extract_bihar.py, extract_kerala.py** (Kerala also aborts rather than write if any pre-existing quarter would vanish). The others remain dangerous — before running ANY per-state extractor, either confirm it upserts or extract to a temp dir and diff. `public/slbc-data/redistribute_date_fields.py` is a completed one-shot migration whose baseline is the RELATIVE ref `HEAD~2` (now unrelated) — it's guarded with a refuse-to-run flag; do not remove the guard.
92. **`public/district_lgd_codes.json` aliases are the cross-state contamination vector, and a bad-fuzzy-match wave keeps recurring.** `match_districts.py` step 4 is an UNCONSTRAINED cross-state alias lookup: a district name that fails its own state's match falls through to whatever state holds that alias. The Gariaband/"Harda" bug (CG's "Gariaband" resolved to MP's Harda, which carried it as an alias) was one of 23 misfiled aliases repaired July 2026 — same signature as the 12 purged in gotcha #53. To audit: replay `DistrictMatcher.resolve()` over every district name in every state's timeseries+complete.json and flag any that resolves to a different `state_lgd_code`; it should be 0. Do NOT trust a fuzzy "alias matches another district better than its holder" screen blindly — it false-flags legitimate renames (Ahilyanagar←Ahmednagar, Subarnapur←Sonepur, Pauri on Pauri Garhwal). `metadata.total_aliases` must be recomputed after any alias edit (it was stale at 109 vs actual 97→93).
93. **Duplicate-header collapse in `_complete.json` — SYSTEMIC, ~160k cells across Bihar/Jharkhand/Odisha.** SLBC merged-header tables repeat sub-column labels (`a`/`pct` for target/achievement/%, `semi_urban`/`urban` under branches/BC/ATMs, `amt`/`Amt`/`amount`, `disbursed_no`/`achv_no`). The quarterly CSVs keep every value positionally (they're lists), but `build_complete_json` keys each district row as a `{field: value}` DICT, so duplicate names overwrite — only the LAST (grand-total) value survives and every per-subcategory value is silently dropped. It propagates into `_fi_timeseries`, `_fi_slim`, the Downloads page and the analysis pages. Blast radius (dropped cells): Bihar ~82k, Jharkhand ~58k, Odisha ~20k, Manipur/Tripura isolated. **Fixed for Odisha only (July 2026):** `slbc-data/odisha/extract_odisha.py:disambiguate_headers()` binds each repeat to the base of its nearest preceding UNIQUE column (`semi_urban`→`branches_semi_urban`, `a`→`total_msme_a`); no-anchor repeats (the sarfaesi CSV is two tables merged) get a positional suffix. `slbc-data/odisha/rebuild_from_quarterly.py` re-derives the 4 canonical artifacts from the intact quarterly CSVs with safety nets (aborts if any surviving value would change or any district set differs). Recovered 20,435 cells, 0 surviving values changed. **Bihar and Jharkhand still carry the bug — apply the same pattern (their duplicate names differ: `amt`/`Amt`/`Amt.`, `achv_no`/`disbursed_no`).**
94. **A duplicate-header fix renames the fields `regenerate_indicator_files_from_states.py`'s fallback chains match, so its fallbacks must learn the recovered names or it blanks the map.** Odisha's map layer is now done: `branch_semi_urban`/`branch_urban` fallbacks already listed `branches_semi_urban`/`branches_urban`, so the regen picked up the recovered branch splits and CORRECTED them (they had been showing the collapsed ATM value); the mudra and sui ACP-total fallbacks were extended with the recovered total field (`total_no_of_a_cs_disbursement_amt` + OCR spelling variants, and the bare `disbursemen_t_amt`/`disburseme_nt_amt` that occur when the OCR typo made the total column unique). Net Odisha map corrections: mudra total preserved (767.72 was already right), sui `sui_o_s_amt` corrected from women-only to the true total (Balangir 9.83→11.51), branch semi-urban/urban from ATM values to branch values (Angul 189→80). Verified 0 regressed-to-None, 0 non-Odisha rows changed. **The OCR splits "disbursement" three ways across quarters (`disbursement`/`disburse_ment`/`disburseme_nt`); this fix enumerates the finite Odisha set in the fallbacks — the truly robust version normalizes those at match time, worth doing when Bihar/Jharkhand bring more.**
