...
94. **After a duplicate-header fix, the regenerator's fallback chains must learn the recovered field names BEFORE `regenerate_indicator_files_from_states.py` is re-run — or it blanks the map.** Disambiguation renames the collapsed bare field the fallbacks matched (e.g. mudra `disbursement_amt` → `total_no_of_a_cs_disbursement_amt`; sui likewise), so a naive regen returns `None` for those Odisha metrics. Some renames happen to match existing fallbacks (`branches_semi_urban` is already there — and picking it up would CORRECT Odisha's branch semi-urban/urban, which currently show the collapsed ATM value on the map). Odisha's ACP totals are extra painful because OCR splits "disbursement" three ways across quarters (`disbursement`/`disburse_ment`/`disburseme_nt`), so a robust fix normalizes those rather than enumerating them in the global fallback lists. **The Odisha fix (gotcha #93) deliberately did NOT re-run the regenerator** — the indicator/map layer is a separate follow-up. Do it before the next `regenerate_indicator_files_from_states.py` run touches Odisha.
